...
export default function FeaturedProduct() {
  ...
  useEffect(() => {
    const fetchEndTime = async () => {
      try {
        const docRef = doc(db, 'settings', 'featuredOffer');
        const docSnap = await getDoc(docRef);
        
        if (docSnap.exists() && docSnap.data().endTime) {
          setEndTime(docSnap.data().endTime);
        } else {
          // Fallback: 3 hours, 9 mins, 57 secs from now (standard users can't write to settings, so we just use it locally if DB is empty)
          setEndTime(Date.now() + (3 * 3600 + 9 * 60 + 57) * 1000);
        }
      } catch (err) {
        console.error("Error fetching offer end time:", err);
        setEndTime(Date.now() + (3 * 3600 + 9 * 60 + 57) * 1000);
      }
    };
    ...
    <div className="shop-featured-banner">
      <div className="container shop-featured-inner">
        <div className="shop-featured-img-box">
          ...
          <img src="https://images.unsplash.com/photo-1593642632823-8f785ba67e45?auto=format&fit=crop&w=800&q=80" alt="Dell Latitude 5420" className="shop-featured-img" fetchPriority="high" decoding="async" width="800" height="450" />
        ...
        <div className="shop-featured-content">
          ...
          <div className="shop-countdown" style={{ display: 'flex', gap: '1rem', marginBottom: '2.5rem' }}>
            <div className="countdown-box" style={{ background: 'var(--gray-light)', padding: '1rem', borderRadius: '8px', textAlign: 'center', minWidth: '70px', color: 'var(--navy)', border: '1px solid #eaeaea' }}><span style={{ fontSize: '1.5rem', fontWeight: 'bold', display: 'block' }}>{formatNumber(timeLeft.days)}</span><div style={{ fontSize: '0.75rem', fontWeight: 'bold' }}>DAYS</div></div>
            <div className="countdown-box" style={{ background: 'var(--gray-light)', padding: '1rem', borderRadius: '8px', textAlign: 'center', minWidth: '70px', color: 'var(--navy)', border: '1px solid #eaeaea' }}><span style={{ fontSize: '1.5rem', fontWeight: 'bold', display: 'block' }}>{formatNumber(timeLeft.hours)}</span><div style={{ fontSize: '0.75rem', fontWeight: 'bold' }}>HRS</div></div>
            <div className="countdown-box" style={{ background: 'var(--gray-light)', padding: '1rem', borderRadius: '8px', textAlign: 'center', minWidth: '70px', color: 'var(--navy)', border: '1px solid #eaeaea' }}><span style={{ fontSize: '1.5rem', fontWeight: 'bold', display: 'block' }}>{formatNumber(timeLeft.minutes)}</span><div style={{ fontSize: '0.75rem', fontWeight: 'bold' }}>MINS</div></div>
            <div className="countdown-box" style={{ background: 'var(--gray-light)', padding: '1rem', borderRadius: '8px', textAlign: 'center', minWidth: '70px', color: 'var(--navy)', border: '1px solid #eaeaea' }}><span style={{ fontSize: '1.5rem', fontWeight: 'bold', display: 'block' }}>{formatNumber(timeLeft.seconds)}</span><div style={{ fontSize: '0.75rem', fontWeight: 'bold' }}>SECS</div></div>
          </div>
          ...
          <div className="shop-featured-actions" style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
            ...
            <button aria-label="Contact us on WhatsApp" style={{ padding: '1rem 2rem', background: '#25D366', color: 'white', border: 'none', borderRadius: '6px', fontWeight: '900', cursor: 'pointer', flex: '1', minWidth: '200px', fontSize: '1.1rem', letterSpacing: '1px' }}>WHATSAPP US</button>
          </div>
        </div>
      </div>
    </div>
